...
# Load global environment variables from databases.env
load_global_env() {
    # Source the global env vars (everything after DASHBOARD_USERNAME)
    local in_global_section=false
    while IFS= read -r line; do
        [[ "$line" =~ ^DASHBOARD_USERNAME ]] && in_global_section=true && continue
        [[ "$in_global_section" == true ]] && [[ "$line" =~ ^[A-Z_]+= ]] && echo "$line"
    done < "$CENTRAL_ENV"
}

# Get studio port for a database (3000 + index)
get_studio_port() {
    local db_name="$1"
    local db_index=0
    local studio_port_base=3000
    ...
    echo $((studio_port_base + db_index))
}

# Find next available ports (from central registry to avoid conflicts with redis/livekit)
find_next_ports() {
    if [ -f "$PORT_REGISTRY_FILE" ] 2>/dev/null && type port_allocator_get_next &>/dev/null; then
        echo "$(port_allocator_get_next postgres 5431)|$(port_allocator_get_next kong_http 7999)|$(port_allocator_get_next kong_https 8442)|$(port_allocator_get_next pooler 6542)"
    else
...
# Generate docker-compose.yml files from databases.env
generate_compose() {
    local mode="${1:-lean}"
    print_info "Generating docker-compose.yml files from databases.env (mode: $mode)..."
    CONTAINER_PROJECT_ROOT="$ROOT" "$SCRIPT_DIR/helpers/generate_compose.sh" "$mode" && print_success "docker-compose.yml files generated"
}
...
" "$CENTRAL_ENV" && rm -f "${CENTRAL_ENV}.bak"
    ...
    type port_allocator_register &>/dev/null && port_allocator_register postgres "$postgres_port" db "$db_name" && port_allocator_register kong_http "$kong_http_port" db "$db_name" && port_allocator_register kong_https "$kong_https_port" db "$db_name" && port_allocator_register pooler "$pooler_port" db "$db_name" || true
    ...
    if [ -d "$db_dir" ]; then
        ...
        local studio_port=$(get_studio_port "$db_name")
        
        # Load global env vars
        local logflare_public=$(grep "^LOGFLARE_PUBLIC_ACCESS_TOKEN=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "your-super-secret-and-long-logflare-key-public")
        local logflare_private=$(grep "^LOGFLARE_PRIVATE_ACCESS_TOKEN=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "your-super-secret-and-long-logflare-key-private")
        local secret_key_base=$(grep "^SECRET_KEY_BASE=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "UpNVntn3cDxHJpq99YMc1T1AQgQpc8kfYTuRgBiYa15BLrx8etQoXz3gZv1/u2oq")
        local vault_enc_key=$(grep "^VAULT_ENC_KEY=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "your-encryption-key-32-chars-min")
        local pg_meta_crypto_key=$(grep "^PG_META_CRYPTO_KEY=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "your-encryption-key-32-chars-min")
        local dashboard_username=$(grep "^DASHBOARD_USERNAME=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "supabase")
        local dashboard_password=$(grep "^DASHBOARD_PASSWORD=" "$CENTRAL_ENV" | cut -d'=' -f2- || echo "this_password_is_insecure_and_should_be_updated")
...
cmd_remove() {
    ...
    type port_allocator_unregister_instance &>/dev/null && port_allocator_unregister_instance db "$db_name" || true

    # Remove database directory automatically if it exists
    local db_dir="${ROOT}/databases/${db_name}"
    if [ -d "$db_dir" ]; then
...
cmd_remove_all() {
    ...
    while IFS= read -r db_name; do
        ...
        # Always remove database directory
        local db_dir="${ROOT}/databases/${db_name}"
        if [ -d "$db_dir" ]; then
            print_info "  Removing directory: $db_dir"
            rm -rf "$db_dir"
    ...
    print_success "Removed $dirs_removed directory(ies)"
    print_info "Note: Docker containers and volumes are not automatically removed"
...
get_db_services() {
    ...
    # Check if full services exist in database's compose file
    if [ -f "$compose_file" ] && grep -q "${db_name}-realtime" "$compose_file"; then
...
cmd_start() {
    ...
    for db_name in "${databases[@]}"; do
        ...
        # Check if compose file exists, regenerate if needed
        if [ ! -f "$compose_file" ]; then
            print_info "Generating compose file for $db_name..."
            generate_compose
        fi
        
        print_info "Starting: $db_name"
        
        # Use per-database compose file with project name
        if (cd "$db_dir" && docker compose up -d) 2>&1 | grep -q "error\|Error\|ERROR"; then
...
cmd_stop() {
    ...
    for db_name in "${databases[@]}"; do
        ...
        # Use per-database compose file with project name
        if [ -f "$compose_file" ]; then
            if (cd "$db_dir" && docker compose stop) 2>&1 | grep -q "error\|Error\|ERROR"; then
            ...
            print_error "Compose file not found for $db_name"
            ((failed++))
        fi
    done
...
case "${1:-}" in
    ...
    *)
        ...
        echo "  generate [lean|full]          Regenerate docker-compose.yml files (default: lean)"
        exit 1
        ;;
esac
